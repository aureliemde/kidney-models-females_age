...
# Called from main.py; returns nothing (cf. initCCD_Var which returns
# hENaC_CCD, hROMK_CCD, hCltj_CCD for use by the flux function).
...
def initCCD(ccd):
    ...
    # ------------------------------------------------------------------
    # Geometry and volumes  (AMW model, AJP Renal 2005)
    # ------------------------------------------------------------------
    SlumPinitccd = 1.2
    SbasPinitccd = 1.2
    SlatPinitccd = 6.9
    ...
    # hENaC_CCD = FM_cENaC * 35.00 * fac4   # dead — not returned (cf. initCCD_Var)
    # hROMK_CCD = 8.0 * fac4                # dead — not returned (cf. initCCD_Var)
    ...
    # NA and K slots are 0 (ENaC/ROMK not assigned here; cf. initCCD_Var)
    # NH4 = h[K, LUM, P] * 0.20 = 0.0 (AMW rule of thumb)
    _h_lum_p = np.array([
        0.0,             # NA    — not set (cf. hENaC_CCD in initCCD_Var)
        0.0,             # K     — not set (cf. hROMK_CCD in initCCD_Var)
        0.80 * fac4,     # CL
        0.16 * fac4,     # HCO3
        PPCh2co3,        # H2CO3
        PPCco2,          # CO2
    ...
    ClTJperm = 1000.0
    # hCltj_CCD = 1.3 * ClTJperm * 1.2   # dead — not returned (cf. initCCD_Var)

    _h_lum_lis = np.array([
        (1.00 / FM_cENaC) * 1.0  * ClTJperm,   # NA
        (1.00 / FM_cENaC) * 1.2  * ClTJperm,   # K
        0.0,                                    # CL   — via hCltj_CCD in initCCD_Var
    ...
    xPendrin = 1.2 * dLA[CL, HCO3, LUM, B] / 1700   # changed for female
