""" Written originally in Fortran by Prof. Aurelie Edwards
 ...
 Initialization of IMCD (inner medullary collecting duct) parameters.
...
def initIMC(imcd):
    """Initialize IMCD parameters: geometry, permeabilities, transporters, inlet conditions.
    ...
    # hENaC_IMC = FM_mENaC * 2.50   # dead — not returned
    # hROMK_IMC = 1.0                # dead — K at LUM-P set directly below
    # fscaleENaC   = 2.50            # dead local — not stored to p or scaling object
    # fscaleNaK    = 1.25            # dead local
    # fscaleNaK_PC = 1.25            # dead local

    # -- LUM-P interface -----------------------------------------------
    # NA: ENaC not set here (cf. hENaC_IMC dead above)
    # K: 1.0 set directly (hROMK_IMC = 1.0 is dead variable; value hardcoded)
    _h_lum_p = np.array([
        0.0,            # NA    — not set (cf. hENaC_IMC)
        1.0,            # K     — directly 1.0 (hROMK_IMC = 1.0 is dead variable)
        2.0e-4,         # CL
        2.0e-4,         # HCO3
        130,            # H2CO3
    ...
    ClTJperm = 1000.0
    # hCltj_IMC = 1.3 * ClTJperm * 1.60   # dead — not returned

    _h_lum_lis = np.array([
        (1.00 / FM_mENaC) * 0.90 * ClTJperm,   # NA
        (1.00 / FM_mENaC) * 0.70 * ClTJperm,   # K
        0.0,                                    # CL   — via hCltj_IMC (dead here)
    ...
    # Note: set_intconc called before reading OMCoutlet (preserve order).
