...
def initOMC_Var(omcd):
    ...
    Returns:
    hENaC_OMC : float
        Basal ENaC expression level for OMCD.
    hROMK_OMC : float
        Basal ROMK expression level for OMCD.
    hCltj_OMC : float
        Basal Cl tight-junction permeability for OMCD.
    """
    ...
    # Use hENaC_OMC as basal value of expression.
    ...
    # Use hROMK_OMC as basal value of expression.
    # omcd(:)%h(2,1,2) = 8.0*fac4 — not used, replaced with hROMK
    hROMK_OMC = 8.0 * fac4
    ...
    # NA: ENaC via hENaC_OMC (not set here)
    # K:  ROMK via hROMK_OMC (not set here)
    # NH4 = h[K, LUM, P] * 0.20 = 0.0 (AMW rule of thumb)
    # GLU = 1.0e-10 — cannot be set to 0 (convergence issue)
    _h_lum_p = np.array([
        0.0,             # NA    — ENaC handled via hENaC_OMC
        0.0,             # K     — ROMK handled via hROMK_OMC
    ...
    # omcd(:)%h(CL,LUM,LIS) = ClTJperm*1.00 — not used; replaced with hCltj_OMC

    _h_lum_lis = np.array([
        (1.00 / FM_mENaC) * 0.80 * ClTJperm,   # NA
        (1.00 / FM_mENaC) * 1.20 * ClTJperm,   # K
        0.0,                                    # CL   — via hCltj_OMC
