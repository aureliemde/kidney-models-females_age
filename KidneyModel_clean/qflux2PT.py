"""
PT (Proximal Tubule) flux evaluator — interior nodes (jz > 0).
...
transporter, and active transport) for PT interior cross-sections (jz > 0).
Called by fcn2PT at each Newton iteration.
...
# LzPT in "main" for "qnewton2PT" equals Lz (Python index, no offset needed)
# PTtorque and fCaTransPT/fCaParaPT are indexed as PTtorque[LzPT+1]


def qflux2PT(x, Cext, EPext, pt, vol0, dcompl, dtorq, Lz, PTinitVol, xNaPiIIaPT, xNaPiIIcPT, xPit2PT):
    """Compute all membrane fluxes for a PT interior node (jz > 0).
    ...
    to transcellular fluxes. Stores select flux values on pt[Lz+1].

    Args:
    Lz : int -- index of the upstream node; current node is pt[Lz+1]

    Returns:
    Jvol : ndarray, shape (NC, NC)        -- volumetric water fluxes
    Jsol : ndarray, shape (NSPT, NC, NC)  -- solute fluxes
    """
    LzPT = Lz   # used for PTtorque and fCaTransPT/fCaParaPT indexing
    ...
    pt[Lz+1].area[LIS, BATH] = pt[Lz+1].sbasEinit * max(Vol[LIS] / pt[Lz+1].volEinit, 1.0)
    pt[Lz+1].area[BATH, LIS] = pt[Lz+1].area[LIS, BATH]
    ...
    Jvol = compute_water_fluxes(C, PM, PbloodPT, Vol, pt[Lz+1].volLuminit,
                                pt[Lz+1].volEinit, pt[Lz+1].volPinit, CPimprefPT,
                                pt[Lz+1].volAinit, CAimprefPT, pt[Lz+1].volBinit,
                                CBimprefPT, pt[Lz+1].area, pt[Lz+1].sig, pt[Lz+1].dLPV,
                                complPT, PTinitVol)
    ...
    convert = href * Cref * np.pi * pt[Lz+1].diam * 60 / 10 * 1.0e9 * Scaletorq

    # --- Electro-convective-diffusive fluxes ---

    Jsol, delmu = compute_ecd_fluxes(C, EP, pt[Lz+1].area, pt[Lz+1].sig, pt[Lz+1].h, Jvol)
    ...
    nagluparam[4] = EP[LUM]
    nagluparam[5] = EP[P]
    nagluparam[6] = pt[Lz+1].CTsglt1

    n = 1
    fluxsglt1 = sglt(n, nagluparam, pt[Lz+1].area[LUM, P])

    Jsol[NA,  LUM, P] += fluxsglt1[0] * pt[Lz+1].xSGLT1
    Jsol[GLU, LUM, P] += fluxsglt1[1] * pt[Lz+1].xSGLT1
    fluxnasglt1  = fluxsglt1[0] * pt[Lz+1].xSGLT1 * convert
    fluxglusglt1 = fluxsglt1[1] * pt[Lz+1].xSGLT1 * convert
    ...
    dJNaglu = (pt[Lz+1].area[LUM, P] * pt[Lz+1].dLA[NA, GLU, LUM, P] *
               (C[NA, LUM] * C[GLU, LUM] * np.exp(zeta) - C[NA, P] * C[GLU, P]) / denom)

    Jsol[NA,  LUM, P] += dJNaglu * pt[Lz+1].xSGLT2
    Jsol[GLU, LUM, P] += dJNaglu * pt[Lz+1].xSGLT2
    fluxnasglt2  = dJNaglu * pt[Lz+1].xSGLT2 * convert
    fluxglusglt2 = dJNaglu * pt[Lz+1].xSGLT2 * convert
    ...
    fluxglut1PE = pt[Lz+1].CTglut1 * pt[Lz+1].area[P, LIS]  * Ro5
    Ro6         = affglut1 * (Gi - Go6) / (affglut1 + Gi) / (affglut1 + Go6)
    fluxglut1PS = pt[Lz+1].CTglut1 * pt[Lz+1].area[P, BATH] * Ro6

    Jsol[GLU, P, LIS]  += fluxglut1PE * pt[Lz+1].xGLUT1
    Jsol[GLU, P, BATH] += fluxglut1PS * pt[Lz+1].xGLUT1
    fluxglut1 = (fluxglut1PE + fluxglut1PS) * pt[Lz+1].xGLUT1 * convert

    # GLUT2
    affglut2    = 17.0
    Ro5         = affglut2 * (Gi - Go5) / (affglut2 + Gi) / (affglut2 + Go5)
    fluxglut2PE = pt[Lz+1].CTglut2 * pt[Lz+1].area[P, LIS]  * Ro5
    Ro6         = affglut2 * (Gi - Go6) / (affglut2 + Gi) / (affglut2 + Go6)
    fluxglut2PS = pt[Lz+1].CTglut2 * pt[Lz+1].area[P, BATH] * Ro6

    Jsol[GLU, P, LIS]  += fluxglut2PE * pt[Lz+1].xGLUT2
    Jsol[GLU, P, BATH] += fluxglut2PS * pt[Lz+1].xGLUT2
    fluxglut2 = (fluxglut2PE + fluxglut2PS) * pt[Lz+1].xGLUT2 * convert

    # --- Luminal NaH2PO4 cotransporter (generic NET formulation) ---

    dJNaP   = pt[Lz+1].area[LUM, P] * pt[Lz+1].dLA[NA, H2PO4, LUM, P] * (delmu[NA, LUM, P] + delmu[H2PO4, LUM, P])
    fluxNaP = dJNaP * convert

    # NaPiIIa: 3 Na+ with 1 HPO4(2-)
    dJNaPiIIa = pt[Lz+1].area[LUM, P] * xNaPiIIaPT * (3 * delmu[NA, LUM, P] + delmu[HPO4, LUM, P])
    ...
    dJNaPiIIc = pt[Lz+1].area[LUM, P] * xNaPiIIcPT * (2 * delmu[NA, LUM, P] + delmu[HPO4, LUM, P])
    dJNaPiIIc *= pt[Lz+1].xSGLT2
    ...
    dJPit2 = pt[Lz+1].area[LUM, P] * xPit2PT * (2 * delmu[NA, LUM, P] + delmu[H2PO4, LUM, P])
    dJPit2 *= pt[Lz+1].xSGLT2
    ...
    dJClBic = pt[Lz+1].area[LUM, P] * pt[Lz+1].dLA[CL, HCO3, LUM, P] * (delmu[CL, LUM, P] - delmu[HCO3, LUM, P])
    ...
    dJClHco2 = pt[Lz+1].area[LUM, P] * pt[Lz+1].dLA[CL, HCO2, LUM, P] * (delmu[CL, LUM, P] - delmu[HCO2, LUM, P])
    ...
    dJH_Hco2 = pt[Lz+1].area[LUM, P] * pt[Lz+1].dLA[H, HCO2, LUM, P] * (delmu[H, LUM, P] + delmu[HCO2, LUM, P])
    ...
    sumJES = 0.0
    for L in (LIS, BATH):
        dJKCl = pt[Lz+1].area[P, L] * pt[Lz+1].dLA[K, CL, P, L] * (delmu[K, P, L] + delmu[CL, P, L])
        Jsol[K,  P, L] += dJKCl
        Jsol[CL, P, L] += dJKCl
        sumJES += dJKCl
    ...
    sumJES = 0.0
    for L in (LIS, BATH):
        dJNaBic = pt[Lz+1].area[P, L] * pt[Lz+1].dLA[NA, HCO3, P, L] * (delmu[NA, P, L] + 3 * delmu[HCO3, P, L])
        Jsol[NA,   P, L] += dJNaBic
    ...
    sumJES = 0.0
    for L in (LIS, BATH):
        dJNDCBE = pt[Lz+1].area[P, L] * pt[Lz+1].dLA[NA, CL, P, L] * (delmu[NA, P, L] - delmu[CL, P, L] + 2 * delmu[HCO3, P, L])
        Jsol[NA,   P, L] += dJNDCBE
        Jsol[CL,   P, L] -= dJNDCBE
    ...
    dJNHEsod, dJNHEprot, dJNHEamm = compute_nhe3_fluxes(C, pt[Lz+1].area[LUM, P], pt[Lz+1].xNHE3)
    ...
    dJactNa5 = pt[Lz+1].area[P, LIS]  * pt[Lz+1].ATPNaK[P, LIS]  * (actNa**3.0) * (actK5**2.0)
    dJactNa6 = pt[Lz+1].area[P, BATH] * pt[Lz+1].ATPNaK[P, BATH] * (actNa**3.0) * (actK6**2.0)
    ...
    dJactH = -pt[Lz+1].area[LUM, P] * pt[Lz+1].ATPH[LUM, P] / DactH
    ...
    AffCa   = 75.6e-6
    dJPMCA5 = pt[Lz+1].area[P, LIS]  * pt[Lz+1].PMCA * C[CA, P] / (C[CA, P] + AffCa)
    dJPMCA6 = pt[Lz+1].area[P, BATH] * pt[Lz+1].PMCA * C[CA, P] / (C[CA, P] + AffCa)

    Jsol[CA, P, LIS]  += dJPMCA5
    Jsol[CA, P, BATH] += dJPMCA6
    fluxPMCA = (dJPMCA5 + dJPMCA6) * convert

    # --- Apply torque scaling to transcellular fluxes (vectorized) ---

    Jsol[:, LUM, P]  *= Scaletorq
    Jsol[:, P, LIS]  *= Scaletorq
    Jsol[:, P, BATH] *= Scaletorq
    ...
    cv  = href * Cref * np.pi * pt[Lz+1].diam * 60 / 10 * 1.0e9   # pmol/min/mm
    cvw = Pfref * Cref * Vwbar * np.pi * pt[Lz+1].diam * 60 / 10 * 1.0e6  # nl/min/mm

    pt[Lz+1].FNatrans  = Jsol[NA,  LUM, P]   * cv
    pt[Lz+1].FNapara   = Jsol[NA,  LUM, LIS] * cv
    pt[Lz+1].FNaK      = fluxNaKsod
    pt[Lz+1].FHase     = fluxHATPase
    pt[Lz+1].FGluPara  = Jsol[GLU, LUM, LIS] * cv
    pt[Lz+1].FGluSGLT1 = fluxglusglt1
    pt[Lz+1].FGluSGLT2 = fluxglusglt2
    pt[Lz+1].FKtrans   = Jsol[K,   LUM, P]   * cv
    pt[Lz+1].FKpara    = Jsol[K,   LUM, LIS] * cv
